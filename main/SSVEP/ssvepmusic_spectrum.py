...
# Hardcoded params
StimuliFilePath = "C:/Users/zkangabci/Git/ssvepspectrum/SSVEPStimuliSpectrum_Data/StreamingAssets/stimuli.json"
ConfigFilePath = "C:/Users/zkangabci/Git/ssvepspectrum/SSVEPStimuliSpectrum_Data/StreamingAssets/config.json"
...
while True:
    ...
    # Remove fake/useless 9th channel
    buffer = buffer[:, :-1]

    # Process EEG buffer
    # No sense for spatial filtering (not even CAR) with this minimal setup
    # Apply forward-backward band-pass filter
    buffer = scipysignal.filtfilt(b, a, buffer, axis=0)
    # Apply DC removal
    buffer = buffer - buffer.mean(axis=0, keepdims=True)
...
#        print(integratedprob)
    ...
    # Detect eye-blinks
    # Isolate only the first channel, and get the last 250 msec (or,
    # the whole buffer if it is less than half second
    if buffer.shape[0] >= int(np.round(ndf.sampling_rate*0.25)):
        eog = buffer[:int(np.round(ndf.sampling_rate*0.25)), 0]
    else:
        eog = buffer[:, 0]

    BlinkValue = np.mean(np.abs(eog)) # Average of last 250 ms on Fz
    ClenchValue = np.mean(np.abs(buffer)) # Average of last 250 ms on all channels
    output = np.concatenate((correlations, np.array([BlinkValue, ClenchValue])), axis=None)

    # Pack and send correlations as OSC messages
    msg = oscbuildparse.OSCMessage("/probs", osctag, output)
