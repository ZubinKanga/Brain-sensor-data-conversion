...
// Layouts
//---------------------------------------------------------------------------
static const std::vector<std::string> electrodeMap_8 =
...
void MainWindow::load_config(const std::string &filename) {
    ...
    try {
        ...
        std::string tmp = pt.get<std::string>("settings.hexEegMask", "0x00000000000000FF");
...
void Reader::OpenEegStream() {
    ...
    */
    ...
    // Simis quick hack of fcbg lsl plugin to work only with a single amp (that I only need)
    // Reason is that I have updated to new SDK where cascaded does not exist
    if(amps.size() == 1 )
        amp = amps.at(0);
    else
        throw(exceptions::notFound("No amp found"));
...
void Reader::read() {
    ...
    try {
        ...
        lsl::stream_info data_info("eego", "EEG", static_cast<int32_t>(channelList.size()) - 1, samplingRate, lsl::cf_double64, "eegoSports_" + amp->getSerialNumber());
