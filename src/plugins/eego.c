...
struct eego_eegdev {
  ...
  struct plugincap* eegocap;

  mm_thread_t thread_id;
  mm_thr_mutex_t acqlock;
  unsigned int runacq;
};
...
// 8-channel custom cap
static label8_t eegolabel8[] = {
    "FCz",   "Cz",    "POz",   "PO3",     "PO4",    "Oz",     "O1",     "O2"};
...
static const char* eegotransducer[] = {"Wet/dry electrode",
                                        "Trigger"};
...
static const struct egdi_signal_info eego_siginfo[2] = {
	{
		.isint = 0, .bsc = 1, .scale = 1.0,
		.dtype = EGD_INT32, .mmtype = EGD_DOUBLE,
		.min.valdouble = -262144.0, .max.valdouble = 262143.96875,
		.unit = &eegounit[0], .transducer = &eegotransducer[0]
	}, {
		.isint = 1, .bsc = 0,
		.dtype = EGD_INT32, .mmtype = EGD_INT32,
		.min.valint32_t = -8388608, .max.valint32_t = 8388607,
		.unit = &eegounit[1], .transducer = &eegotransducer[1]
	}
};
...
static void initialize_amplifiers(struct eego_eegdev* eegodev) {
  ...
  /* Removing support for cascaded amplifiers as it seems to not be handled (at least not in this way)
  from the newer eego-SDK API. Working with the first connected amplifier only
...
static int eego_set_capability(struct eego_eegdev* eegodev,
                               const char* optv[]) {
  // Simis update: From the original version of this plugin, one
  // already has the number, types, labels, etc. of channels since
  // before the present function eego_set_capability is called, functions
  // get_channel_list and get_label have been already called by eego_open_device
  // and have saved all this info in the eegodev dev structure. Here, I simply create using
  // this "old" info the mappings and plugincap settings (e.g. chmap) to comply with the
  // new EEGDEV ABI VERSION
  
  //Allocate memory for chmap
  eegodev->chmap = malloc(eegodev->NCH*sizeof(*eegodev->chmap));
  eegodev->eegocap = malloc(sizeof(*eegodev->eegocap));
  
  struct blockmapping mappings = {.num_skipped = 0};
  
  unsigned int chind = 0;
  for (int i=0; i<eegodev->NUM_EEG_CH; i++) {
	eegodev->chmap[chind].stype = EGD_EEG;
	eegodev->chmap[chind].label = eegodev->eeglabel[i];
	eegodev->chmap[chind].si = NULL;//&eego_siginfo[0];
	chind++;
  }
  for (int i=0; i<eegodev->NUM_EXG_CH; i++) {
	eegodev->chmap[chind].stype = EGD_SENSOR;
	eegodev->chmap[chind].label = eegodev->sensorlabel[i];
	eegodev->chmap[chind].si = NULL;//&eego_siginfo[0];
	chind++;
  }
  for (int i=0; i<eegodev->NUM_SAMPLE_COUNTER_CH+eegodev->NUM_TRI_CH; i++) {
	eegodev->chmap[chind].stype = EGD_TRIGGER;
	eegodev->chmap[chind].label = trigglabel;
	eegodev->chmap[chind].si = NULL;//&eego_siginfo[1];
	chind++;
  }
  mappings.nch = eegodev->NCH;
  mappings.chmap = eegodev->chmap;
  int SR = atoi(optv[0]);
  const char* devid = optv[3];
  ...
  eegodev->eegocap = &cap;
  dev->ci.set_cap(dev, eegodev->eegocap);
...
static int prepareMask(struct eego_eegdev* eegodev, const char* optv[]) {
  ...
  if (strcmp(optv[1],"NOMASK") != 0) {
	...
	// 8 custom cap
	if (strcmp(optv[3], "8") == 0) {
	  eegodev->eegolabel = &eegolabel8;
	}
    // 64 CA-200 cap
    else if (strcmp(optv[3], "200") == 0) {
    ...
    // 8 custom cap
    if (strcmp(optv[3], "8") == 0) {
      eegodev->ref_mask = (unsigned long long) 0xFF; //0xFF;
      eegodev->eegolabel = &eegolabel8;
    }    
    // 64 CA-200 cap
    else if (strcmp(optv[3], "200") == 0) {
...
static int eego_open_device(struct devmodule* dev, const char* optv[]) {
  ...
  mm_thr_mutex_init(&(eegodev->acqlock), NULL);
  eegodev->runacq = 1;
  int ret;
  if ((ret =
           mm_thr_create(&(eegodev->thread_id), eego_read_fn, eegodev)))
    goto error;
...
static int eego_close_device(struct devmodule* dev) {
  ...
  mm_thr_mutex_lock(&eegodev->acqlock);
  eegodev->runacq = 0;
  mm_thr_mutex_unlock(&eegodev->acqlock);
  ...
  free_label(eegodev);
  free(eegodev->chmap);
  free(eegodev->eegocap);
  
  mm_thr_join(eegodev->thread_id, NULL);
  mm_thr_mutex_deinit(&eegodev->acqlock);
  eemagine_sdk_exit();
  return 0;
}
 ...
 * @param      info   The egdi_chinfo to fill.
 */
